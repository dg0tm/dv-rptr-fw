...
static const unsigned short tab_ccitt_revers[256] = {
...
unsigned short crc_ccitt(const char *buffer, unsigned int len) {
  ...
  for (; len>0; len--) {
#ifdef USECCITTTABLE
    // CRC  =  LoByteCRC SHL 8   XOR Tabelle[HiByteCRC XOR Data]
    crc.u16 = ((U16)crc.u8[1]<<8) ^ tab_ccitt[crc.u8[0]^(*buffer++)];
#else
    data = *buffer++;
    for (i=0; i<8; i++) {
      tmp  = data ^ crc.u8[0];	// HiByte
      crc.u16 <<= 1;
      if (tmp&0x80) crc.u16 = (crc.u16 ^ 0x1020) | 1;
      data <<= 1;
    }
#endif
  } //for i
...
void append_crc_ccitt(char *buffer, unsigned int len) {
  ...
  *buffer++ = crc.u8[0];	// Hi-Byte CRC
  *buffer   = crc.u8[1];	// LO-Byte CRC
}
...
void append_crc_ccitt_revers(char *buffer, unsigned int len) {
  ...
  crc.u16 = 0xFFFF;
  for (; len>2; len--) {
    crc.u16 = (U16)crc.u8[0] ^ tab_ccitt_revers[crc.u8[1]^(*buffer)];
    buffer++;
  }
  crc.u16   = ~crc.u16;
  *buffer++ = crc.u8[1];	// HI-Byte CRC
  *buffer   = crc.u8[0];	// LO-Byte CRC
}
